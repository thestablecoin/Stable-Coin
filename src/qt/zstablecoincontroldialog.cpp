...
#include "ui_zstablecoincontroldialog.h"
...
std::list<std::string> ZStableCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZStableCoinControlDialog::listMints;

ZStableCoinControlDialog::ZStableCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZStableCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZStableCoinControlDialog::~ZStableCoinControlDialog()
{
    delete ui;
}

void ZStableCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZStableCoinControlDialog::updateList()
{
...
void ZStableCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZStableCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZStableCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZStableCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZStableCoinControlDialog::GetSelectedMints()
{
...
void ZStableCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
