...
#include "zstablecoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSTABL ought to be enough for anybody." - Bill Gates, 2017
    ui->zSTABLpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSTABLSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzSTABL->setEnabled(false);
        ui->pushButtonMintzSTABL->setToolTip(tr("zSTABL is currently disabled due to maintenance."));

        ui->pushButtonSpendzSTABL->setEnabled(false);
        ui->pushButtonSpendzSTABL->setToolTip(tr("zSTABL is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSTABLpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSTABL_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSTABL is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSTABL...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSTABL in ") +
...
void PrivacyDialog::on_pushButtonSpendzSTABL_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSTABL is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSTABL
        sendzSTABL();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSTABL
    sendzSTABL();
}

void PrivacyDialog::on_pushButtonZStableCoinControl_clicked()
{
    ZStableCoinControlDialog* zStableCoinControl = new ZStableCoinControlDialog(this);
    zStableCoinControl->setModel(walletModel);
    zStableCoinControl->exec();
}

void PrivacyDialog::setZStableCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzStableCoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSTABL()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid StableCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSTABLpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSTABL is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " STABL </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSTABL</b>";
    ...
    // use mints from zStableCoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZStableCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZStableCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zstablecoin selector in case it was used
    ZStableCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zStableCoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " StableCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zStableCoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSTABL </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSTABL "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSTABL "));
    ui->labelzSTABLAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSTABLSyncStatus->setVisible(fShow);
}
